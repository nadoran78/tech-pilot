...
2026-09-24에 로컬에서 실제 수동 수집을 검증했다.

- `robots.txt`는 `User-agent: *`에 대해 `Allow: /`를 계속 제공했고, Terms of Service 페이지는 HTTP 200으로 접근됐다. 이는 당시의 접근 가능성 확인일 뿐, 약관의 의미나 콘텐츠 재게시 권한을 새로 판단한 것은 아니다.
- 첫 요청은 HTTP 200으로 완료되어 867개 항목을 저장했고, 제외·중복·검토 필요 항목은 없었다.
- 같은 로컬 DB로 즉시 다시 요청했을 때 HTTP 304로 변경 없음을 확인했다. 이 결과는 조건부 요청이 당시 endpoint에서 동작했음을 보여 주지만, 지속적인 응답 계약을 보장하지는 않는다.
- 수집 항목의 품질 검토와 안전한 재현 절차는 [뉴스 수집 수동 운영 runbook](../runbooks/news-collection.md)을 따른다.
