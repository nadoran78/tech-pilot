...
- **Date:** 2026-09-03
- **Phase:** 2
- **Related documents:** [기술 스택 기준](../architecture/tech-stack.md), [2단계 문서](../phases/02-tech-stack-selection.md), [결정 기록 안내](README.md)
...
자동 실행은 수동 CLI가 신뢰 가능한 fixture와 저장 계약을 통과한 뒤 로컬 스케줄러로 검토한다. 정확한 Python 마이너 버전은 [결정 기록 0002](0002-python-3-13-bootstrap-runtime.md)에서 별도로 검토하며, 스케줄러는 이후 수집 설계 Issue에서 결정한다.

## Options Considered

| 선택지 | 평가 |
|---|---|
| Python CLI + SQLite | 수집·파싱·데이터 처리에 직접적이고 별도 서버가 없어 MVP 복잡도가 낮다. 채택한 기본안이다. |
...
- 다음 작업: 초기화 Issue에서 정확한 Python 버전, 도구 설정과 디렉터리 구조를 구현한다.
